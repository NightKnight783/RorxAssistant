...
module.exports = {
  ...
  async execute (message) {
    ...
    db.serialize(() => {
      db.get('SELECT * FROM members_xp WHERE userId = ?', [message.author.id], (error, value) => {
        if (error) {
          console.error(error)
          return
        }
        ...
        if (!value) {
          ...
          db.run('INSERT into members_xp (userId, userName, time) values (?, ?, ?)', [message.author.id, message.author.globalName, Date.now()])
        } else {
          ...
          // Update user xp
          db.run('UPDATE members_xp SET xp = ?, level = ?, userName = ?, time = ? WHERE userId = ?', [xp, level, message.author.globalName, Date.now(), value.userId])
        }
