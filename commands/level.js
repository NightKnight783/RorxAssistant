...
module.exports = {
  ...
  async execute (interaction) {
    ...
    db.serialize(() => {
      db.get('SELECT * FROM members_xp WHERE userId = ?', [interaction.user.id], async (error, value) => {
        if (error) {
          console.error(error)
          return
        }
        ...
        if (!value) {
          ...
          db.run('INSERT into members_xp (userId, userName, time) values (?, ?, ?)', [interaction.user.id, interaction.user.globalName, Date.now()])
